...
# FIX #4 — batch size protection: chunk large user lists before RPC calls
# to prevent memory spikes and oversized payloads at scale.
RECOVERY_CHUNK_SIZE = int(os.getenv("RECOVERY_CHUNK_SIZE", "500"))
...
class RiskSignal(StrEnum):
    ...
    def from_raw(cls, raw_value: Any) -> Optional["RiskSignal"]:
        ...
        try:
            ...
            # FIX #10 — downgraded to DEBUG: unknown signals from user-supplied
            # data can be extremely noisy at scale and are not actionable as warnings.
            logger.debug("recovery_unknown_risk_signal raw=%s", raw_value)
            return None
    ...
    def bulk_dispatch_candidates(
        ...
        try:
            ...
            if os.getenv("RECOVERY_RAISE_ON_BULK_RPC_FAILURE", "").strip().lower() in {"1", "true", "yes"}:
                raise
            # FIX #5 — emit a distinct metric so dashboards can alert when the
            # system silently degrades to empty results instead of raising.
            METRICS.increment("recovery.bulk_rpc.soft_fallback", 1, {"tenant": tenant_id})
...
class RecoveryAutomationEngine:
    ...
    def evaluate_and_queue_batch(
        ...
        for user in users:
            ...
            # FIX #3 — use a sentinel for None run_id to avoid key collisions
            # across batches that share tenant + user + campaign but have no run_id.
            dedup_key = (
                f"{tenant_id}:{run_id or 'no_run'}:"
                f"{candidate.user_id}:{candidate.campaign_type.value}"
            )
        ...
        # FIX #4 — chunk RPC calls to cap payload size and avoid memory spikes.
        # Candidate building is done upfront so global deduplication is preserved.
        all_decisions: List[RecoveryDecision] = []
        all_outcome_counts: Dict[str, int] = {}

        for chunk_start in range(0, len(valid_candidates), RECOVERY_CHUNK_SIZE):
            chunk = valid_candidates[chunk_start:chunk_start + RECOVERY_CHUNK_SIZE]
            response = self.repo.bulk_dispatch_candidates(tenant_id, chunk, run_id)
            all_decisions.extend(_convert_bulk_results(response.results))

            for outcome, count in _count_outcomes(response.results).items():
                all_outcome_counts[outcome] = all_outcome_counts.get(outcome, 0) + count
                METRICS.increment(
                    "recovery.bulk.outcome", count, {"tenant": tenant_id, "outcome": outcome}
                )
    ...
    def _build_candidate(
        ...
        # FIX #6 — pre-validate email structure before handing off to Pydantic,
        # giving a targeted rejection path for malformed addresses (e.g. multiple @).
        safe_email = _normalize_email(raw_email)
        if safe_email is None:
            logger.warning(
                "recovery_candidate_rejected tenant=%s run_id=%s user_id=%s reason=invalid_email_format",
                tenant_id,
                run_id,
                user_id,
            )
...
class OutboxDispatcher:
    ...
    Race-condition contract (FIX #7):
        Queue send is attempted first. If the send succeeds but the subsequent
        DB status update fails, the row remains claimable and will be retried on
        the next cycle. To prevent duplicate email delivery, the downstream worker
        MUST treat ``dispatch_token`` as an idempotency key and reject already-seen
    ...
    def process_pending_outbox(self, limit: int = 100) -> None:
        ...
        for row in rows:
            # FIX #8 — per-row timing so slow outliers are visible in dashboards.
            ...
            try:
                # Enqueue first. If this succeeds but the DB update below fails,
                # the row will be re-claimed next cycle — the worker deduplicates
                # via dispatch_token (see class docstring).
                ...
                # FIX #7 — DB update is a separate inner try so a failure here
                # doesn't trigger _mark_dispatch_failed (the message is already live).
                try:
                    ...
                    logger.error(
                        "recovery_dispatch_db_update_failed tenant=%s send_id=%s error=%s",
                        tenant_id,
                        send_id,
                        db_exc,
                    )
                    METRICS.increment(
                        "recovery.outbox.db_update_failed", 1, {"tenant": tenant_id}
                    )

            except Exception as exc:
                logger.error(
                    "recovery_dispatch_failed tenant=%s send_id=%s attempt=%d error=%s",
                    tenant_id,
                    send_id,
                    dispatch_attempt,
                    exc,
...
def _parse_bulk_dispatch_response(
    ...
    try:
        ...
        # FIX #2 — catch all exceptions, not just ValidationError.
        # TypeAdapter can also raise TypeError or ValueError on malformed input.
...
def _count_outcomes(results: List[BulkDispatchResult]) -> Dict[str, int]:
    counts: Dict[str, int] = {}
    for result in results:
        # FIX #1 — guard against raw strings arriving from JSON before Pydantic
        # coercion completes; .value raises AttributeError on plain str.
        key = result.outcome.value if isinstance(result.outcome, Enum) else str(result.outcome)
...
def _normalize_email(raw_email: str) -> Optional[str]:
    """Normalize and structurally pre-validate an email string.

    Returns ``None`` for obviously malformed addresses (zero or multiple ``@``
    signs, empty local part, missing dot in domain) so that invalid emails are
    rejected early with a targeted log entry rather than as an opaque Pydantic
    ``ValidationError``.  Full RFC validation is intentionally left to
    Pydantic's ``EmailStr``.

    FIX #6 — previous implementation returned a plain ``str`` with no structural
    checks, allowing malformed inputs (e.g. ``a@b@c.com``) to reach Pydantic
    without any prior signal in the logs.
    """
    compact = unicodedata.normalize("NFKC", str(raw_email).strip().replace(" ", "")).lower()
...
def _log_validation_failure(
    ...
    # FIX #11 — build dot-notation paths from the full loc tuple so nested field
    # errors are human-readable (e.g. "address.zip" instead of just "zip").
    fields = [
        ".".join(map(str, e["loc"]))
        for e in exc.errors()

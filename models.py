...
class Agent(Base, TenantAwareMixin):
    ...
    Phase 1 Update: Strictly enforces 1-to-1 data mapping.
    """
    __tablename__ = "agents"

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    
    # --- Phase 1: 1-to-1 Boundary Limits ---
    # Ripped out JSON arrays. An agent is tied to ONE structured dataset OR ONE document.
    dataset_id: Mapped[Optional[uuid.UUID]] = mapped_column(ForeignKey("datasets.id", ondelete="CASCADE"), nullable=True)
    document_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), nullable=True) # Unstructured Qdrant Pointer
    ...
    investigations: Mapped[List["InvestigationRecord"]] = relationship("InvestigationRecord", back_populates="agent", cascade="all, delete-orphan")


class InvestigationRecord(Base, TenantAwareMixin):
    """
    Phase 1: Deep Memory Persistence.
    Stores the executive summaries generated by the background autonomous pipeline.
    """
    ...
    severity_level: Mapped[str] = mapped_column(String, nullable=False) # e.g. "Low", "Medium", "High"
    ...
    # Relationships
    agent: Mapped["Agent"] = relationship("Agent", back_populates="investigations")

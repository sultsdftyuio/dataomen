...
export const POST = withTenant(async (req, { supabase, tenantId, userId }) => {
  ...
  const { error } = await (supabase as any).rpc("skip_queue_item_intervention", {
  ...
  if (error) {
    console.error("[QUEUE_SKIP] failed", { tenantId, itemId, userId, error });
    return NextResponse.json(
      { error: error.message ?? "Failed to skip queue item" },
      { status: 409 }
    );
  }

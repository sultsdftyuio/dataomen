...
export const POST = withTenant(async (req, { supabase, tenantId, userId }) => {
  ...
  const { error } = await (supabase as any).rpc("execute_queue_item_intervention", {
  ...
  if (error) {
    console.error("[QUEUE_EXECUTE] failed", { tenantId, itemId, userId, error });
    return NextResponse.json(
      { error: error.message ?? "Failed to execute queue item" },
      { status: 409 }
    );
  }

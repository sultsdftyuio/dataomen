...
function SettingsPageContent() {
  ...
  // Phase 3: Real Tenant Settings State
  const [notifyAnomalies, setNotifyAnomalies] = useState(true);
  const [notifyWeekly, setNotifyWeekly] = useState(true);
  const [initialNotifyAnomalies, setInitialNotifyAnomalies] = useState(true);
  const [initialNotifyWeekly, setInitialNotifyWeekly] = useState(true);
  ...
  // Load User Profile AND Tenant Settings securely without passing tenant_id
  useEffect(() => {
    let mounted = true;
    ...
    async function loadData() {
      ...
      try {
        ...
        // 2. Load Tenant Settings (Phase 3 Fix)
        const settingsRes = await fetch('/api/settings');
        if (settingsRes.ok) {
          const settings = await settingsRes.json();
          if (mounted) {
            setNotifyAnomalies(settings.notifyAnomalies ?? true);
            setNotifyWeekly(settings.notifyWeekly ?? true);
            setInitialNotifyAnomalies(settings.notifyAnomalies ?? true);
            setInitialNotifyWeekly(settings.notifyWeekly ?? true);
            setApiKey(settings.apiKey || "No active key");
            setKeyLastUpdated(settings.keyLastUpdated || "Unknown");
          }
        }
      } catch (error) {
  ...
  const handleSaveNotifications = async () => {
    ...
    try {
      const res = await fetch('/api/settings/notifications', {
        ...
        // Phase 3: Server resolves tenant context safely
  ...
  const handleRegenerateApiKey = async () => {
    // Determine user intent for idempotency/safety
    if (!confirm("WARNING: Regenerating this key will instantly break any existing API integrations using the old key. Proceed?")) return;
  ...
  if (isLoading) return <SettingsPageFallback />;
  ...
  return (
    <div className="flex flex-col flex-1 h-full min-h-[calc(100vh-4rem)] w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="mb-6 px-2">
        ...
        <p className="text-sm text-slate-500 mt-1">Manage workspace configuration, access controls, and integration protocols.</p>
      </div>

      {/* Main Unified Interface Container */}
      <div className="flex flex-col md:flex-row flex-1 w-full bg-white border border-slate-200/80 rounded-xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] overflow-hidden">
        ...
        <aside className="w-full md:w-72 shrink-0 bg-slate-50/50 border-r border-slate-100 flex flex-col justify-between z-10">
          <div className="p-4 space-y-1">
            <div className="text-[10px] font-bold tracking-wider text-slate-400 uppercase mb-3 px-3">Configuration Map</div>
            {TABS.map((tab) => {
              const Icon = tab.icon;
        ...
        <main className="flex-1 flex flex-col relative bg-gradient-to-br from-white to-slate-50/30 overflow-hidden w-full">
          ...
          {activeTab === "notifications" && (
            <div className="flex flex-col h-full animate-in fade-in duration-300">
              ...
              {/* Phase 3: Added Missing Action Bar for Routing Settings */}
              <div className="px-8 lg:px-12 py-5 bg-slate-50/80 border-t border-slate-100 flex items-center justify-between mt-auto">
            ...
            <div className="flex flex-col h-full animate-in fade-in duration-300">
              ...
              <div className="p-8 lg:px-12 flex-1 overflow-y-auto space-y-10">
                <div className="space-y-4 max-w-3xl">
                  ...
                  {/* Phase 3: Connected Regeneration Action */}

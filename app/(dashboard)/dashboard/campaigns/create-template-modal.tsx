...
import { EmailTemplate } from "./campaigns-client";
      ...
      <DialogContent className="sm:max-w-[500px] border-slate-200 shadow-xl rounded-xl">
        <form onSubmit={handleCreateTemplate}>
          <DialogHeader className="px-6 py-5 border-b border-slate-100 bg-slate-50/50">
            <DialogTitle className="text-lg text-slate-900 tracking-tight">New Recovery Template</DialogTitle>
            <DialogDescription className="text-sm text-slate-500 mt-1.5">
              Draft a personalized template sequence. Placeholders like {'{{first_name}}'} will be injected dynamically at send-time.
            </DialogDescription>
          </DialogHeader>
          
          <div className="px-6 py-6 space-y-5 bg-white">
            <div className="space-y-2.5">
              <Label htmlFor="tpl-name" className="text-xs font-bold uppercase tracking-wide text-slate-600">
                Template Name
              </Label>
              <Input 
                id="tpl-name" 
            ...
            <div className="space-y-2.5">
              <Label htmlFor="tpl-subj" className="text-xs font-bold uppercase tracking-wide text-slate-600">
                Email Subject Line
              </Label>
              <Input 
                id="tpl-subj" 
          ...
          <DialogFooter className="px-6 py-4 border-t border-slate-100 bg-slate-50">
            ...
            >
              {isCreating ? <RefreshCw className="h-4 w-4 animate-spin" /> : "Save Template"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
